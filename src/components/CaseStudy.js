    ...
    <div className="flex">
      ...
      <div className="xl:w-4/5 xl:pt-0">
        <div id="case-study" className="prose ml-6 leading-9 w-10/12 text-left text-gray-700 text-xl">
          ...
          <p className={paragraphStyle}>
            Boardwalk is a small, but fast-growing online retailer that sells handcrafted board games. Their product has become a huge hit on social media, which has exponentially increased the traffic on their website. This is an exciting time for Boardwalk, but also a challenging time, since their growth will require a complete overhaul of their architecture.
          </p>
          <p className={paragraphStyle}>
          ...
          <img src={monolithDiagram} alt="monolith diagram" className={imgStyle} />
          <p className={paragraphStyle}>
          ...
          <img src={microserviceDiagram} alt="microservice diagram" className={imgStyle} />
          <p className={paragraphStyle}>
            Services must now communicate by making API calls over an inherently unreliable network.
          ...
          <p className={paragraphStyle}>
            In their previous architecture, the Boardwalk team needed only to SSH into their application server, and maybe their database server, to examine their application's logs.
          </p>
          <img src={monolithSSH} alt="SSHing into a monolith" className={imgStyle} />
          <p className={paragraphStyle}>
            Now, they have to SSH into multiple instances, examine their logs, and try to piece together the entire application state from a series of isolated records.
          </p>
          <img src={microserviceSSH} alt="SSHing into a microservice" className={imgStyle} />
          <p className={paragraphStyle}>
            Is the checkout service running? Is the payment service running? Are they both running but the network is failing? Fortunately, the number of nodes is still low enough that it’s possible, but as it increases, they can see that this process will soon become untenable.
          </p>
          ...
          <img src={logsAggregated} alt="Aggregated logs" className={imgStyle} />
          <p className={paragraphStyle}>
          ...
          <p className={paragraphStyle}>
            A mature observability solution needs to store the data in a way that it is queryable and visualizable. In SQL database terms, this process would entail dividing up the relevant characteristics of the logs, such as the timestamp, the source, and the message, and organizing the tables and columns of the database using these characteristics. That way, instead of visually scanning through thousands of lines of text, they can leverage all the power of SQL queries to find and analyze the logs they’re looking for.
          </p>
          <img src={logsNormalized} alt="Aggregated, normalized logs" className={imgStyle} />
          <p className={paragraphStyle}>
            In addition to being searchable, the data returned from these queries can now be fed into a visualization tool to create interactive dashboards. This is the path for getting from raw text stored in multiple locations to an interactive dashboard summarizing all of the data in one location. There is a bit more left to learn about logs, though, before the team can decide on a solution.
          </p>
          <h3 id="section-2-5" className={subHeaderStyle}>2.5 Time-Series Data</h3>
          <p className={paragraphStyle}>
            One important factor they need to consider is the time-series nature of log data. This means the time the log occurred is a primary component in organizing it.
          </p>
          <div className="h-72 overflow-hidden">
            <img src={timeSeries} alt="timeseries data" className="mx-auto -mt-2" />
          </div>
          <p className={paragraphStyle}>
            If the team needs to diagnose a failure that occurs at 4:30pm, they need to see the logs generated in a narrow window of time leading up to and immediately after that time.
          </p>
          <p className={paragraphStyle}>
          ...
          <p className={paragraphStyle}>
            While things are running well, logs tend to be generated in a fairly steady stream. However, when something goes wrong, systems can generate as much as 5 times the normal amount. If a logging solution is not prepared to handle those bursts, it can fail when it is needed most.
          </p>
          <h3 id="section-2-7" className={subHeaderStyle}>2.7 Summary</h3>

          <ul className={listDiscStyle}>
            <p className={listTextStyle}>In summary, the team needs a solution that:</p>
            <li className={listElementStyle}><p className={listTextStyle}>Normalizes the logs</p></li>
            <li className={listElementStyle}><p className={listTextStyle}>Collects the logs from all nodes</p></li>
            <li className={listElementStyle}><p className={listTextStyle}>Stores the logs in a central location</p></li>
            <li className={listElementStyle}><p className={listTextStyle}>Queries and visualizes the logs</p></li>
            <li className={listElementStyle}><p className={listTextStyle}>Manages the storage of the logs based on relevance</p></li>
            <li className={listElementStyle}><p className={listTextStyle}>Handles bursts of logs</p></li>
          </ul>
          ...
          <img src={buyingOptions} alt="the logos of existing solutions" className={`${imgStyle} w-8/12`} />
          <p className={paragraphStyle}>
            There are great log management services in the marketplace such as <a href="https://logz.io/" className={linkStyle}>Logz</a>, <a href="https://www.logdna.com/" className={linkStyle}>LogDNA</a>, and <a href="https://www.scalyr.com/" className={linkStyle}>Scalyr</a>. However, their convenience comes with a steep cost and the potentially sensitive data being sent to them is no longer owned by their users.
          </p>
          <p className={paragraphStyle}>
            AWS, like most cloud providers, does offer a <a href="https://docs.aws.amazon.com/cloudtrail/" className={linkStyle}>ready-to-deploy logging pipeline</a> consisting of managed services for ingesting, storing, and visualizing logs. This solution allows users to have the convenience of a managed solution while maintaining data ownership, but each component in the pipeline charges per log that passes through it, which, as log traffic increases, can potentially be even more expensive than the third-party managed solutions.
          </p>
          ...
          <p className={paragraphStyle}>
            Logstash, in addition to parsing and indexing data, is used to serve as a collection and shipping agent, but the Elastic team has since replaced it with their lighter weight tool Beats. The inclusion of Beats in the ELK stack is why Elastic now refers to it as the Elastic stack, which is the term we will use going forward.
          </p>
          <p className={paragraphStyle}>
          ...
          <img src={loggingSolutions} alt="solution comparison" className={`${imgStyle} -mb-10`} />
          <p className={paragraphStyle}>
          ...
          <img src={overview} alt="An overview of Lodge" className={`${imgStyle} max-w-xl -mb-8`} />
          <p className={paragraphStyle}>
            The user has deployed Lodge on their network, so now all the applications in that network can ship logs to the stack using Filebeat, a subset of Beats specifically for collecting and forwarding log data. This user can then view those logs from the Lodge dashboard that is deployed with the stack.
          </p>
          <p className={paragraphStyle}>
          ...
          <p className={paragraphStyle}>
            Lodge’s infrastructure consists of a series of components that work together to ship, transform, store, visualize,  and monitor log data.
          </p>
          <img src={components} alt="The components of Lodge" className={imgStyle} />
          <p className={paragraphStyle}>
            Some of these components will be familiar from our previous discussion of the Elastic stack. Lodge uses these components as well as some additional supporting ones, all of which we will go over before we demonstrate how Lodge works.
          </p>
          ...
          <img src={filebeat} alt="Filebeat in the Lodge stack" className={imgStyle} />
          <p className={paragraphStyle}>
          ...
          <img src={kafka} alt="Kafka in the Lodge stack" className={imgStyle} />
          <p className={paragraphStyle}>
          ...
          <img src={logstash} alt="Logstash in the Lodge stack" className={imgStyle} />
          <p className={paragraphStyle}>
            In this case, Logstash first ingests log data from specific Kafka topics, performs parsing and transformation, and sends logs to two different storage components in Lodge.
          </p>
          <h3 className={subHeaderStyle}>4.6 Elasticsearch and Amazon S3 for Storage</h3>
          <p className={paragraphStyle}>
            This leads us to the storage layer. Here, in addition to Elasticsearch, we have an Amazon S3 bucket receiving data from Logstash.
          </p>
          ...
          <img src={restore} alt="Lodge Restore in the Lodge stack" className={imgStyle} />
          <p className={paragraphStyle}>
            As we mentioned previously, S3 stores data that is no longer needed in Elasticsearch. But what if the user changes their mind? For that purpose, we built an application to retrieve data from S3 and re-index it back into Elasticsearch.
          </p>
          <h3 id="section-4-6" className={subHeaderStyle}>4.7 Kibana for Querying and Visualization</h3>
          <p className={paragraphStyle}>
            Kibana is the remaining Elastic stack component we use in Lodge. Kibana is Elastic’s UI for interacting with Elasticsearch through its API. Kibana stores its data within Elasticsearch indices, so we can keep Kibana stateless.
          </p>
          <img src={kibana} alt="kibana in the Lodge stack" className={imgStyle} />
          <p className={paragraphStyle}>
            If the Kibana server goes down, users can simply spin up a new one without worrying about losing data - and this convenience comes without the hassle of managing a separate database.
          </p>
          ...
          <img src={dashboard} alt="the Lodge dashboard in the Lodge stack" className={`${imgStyle} max-w-xl`} />
          ...
          <ul className={listNumStyle}>
            ...
            <li className={`${listElementStyle} pl-4`}>
              <p className={listTextStyle}>Once the installation is done, the user can run <code>lodge init</code> to initialize Lodge and prepare it to be deployed on AWS.</p>
            </li>
            <li className={`${listElementStyle} pl-4`}>
              <p className={listTextStyle}>Once the initialization is done, the user can run <code>lodge deploy</code>, which after a few questions, it will deploy the entire Lodge infrastructure based on a series of Cloudformation templates that are generated from the Lodge CDK application, onto either a new AWS VPC or an existing VPC of their choice.</p>
            </li>
            <li className={`${listElementStyle} pl-4`}>
              <p className={listTextStyle}>As this is a large, complex deployment, it may take quite some time to finish.</p>
            </li>
          </ul>

          <h3 id="section-5-2" className={subHeaderStyle}>5.2 Viewing the Deployment</h3>
          <p className={paragraphStyle}>
            Lodge is deployed on a total of 15 AWS EC2 instances that can be viewed and interacted with from the AWS console, if the user wishes to do so.
          </p>
          <img src={awsConsole} alt="The aws console, showing Lodge deployed" className={imgStyle} />
          <p className={paragraphStyle}>
            We can see in the AWS console that some of the components are deployed in multiple instances and in autoscaling groups with each instance in a different availability zone. The reasoning behind these design decisions, as well as the component we have not yet introduced, the bastion host, will be discussed in the next section. First, let’s see what it looks like to use Lodge.
          </p>
          ...
          <dl>
            ...
            <div className="flex mb-2">
              ...
              <dd className="inline">Kafka writes data to disk, so if a node crashes, data is not lost.</dd>
            </div>
            <div className="flex mb-2">
              <dt className="font-bold inline mr-8">Availability:</dt>
              <dd className="inline">Kafka replicates data across brokers, so if one broker goes down, its data is still accessible from the other brokers.</dd>
            </div>
            <div className="flex mb-2">
              <dt className="font-bold inline mr-4">Performance:</dt>
              <dd className="inline">Kafka is able to process a very high volume of messages very quickly, which makes it able to handle bursts in logs when they occur.</dd>
            </div>
          </dl>
          ...
          <img src={elasticsearchCluster} alt="The elasticsearch cluster" className={imgStyle} />
          <p className={paragraphStyle}>
            Some of the specific recommendations we implemented were as follows:
          </p>
          <p className={paragraphStyle}>
            Separating master-eligible and data node roles to dedicated instances so the tasks of managing cluster state and disk I/O operations do not compete for resources and they can scale independently.
          </p>
          <p className={paragraphStyle}>
          ...
          <img src={circularDependencies} alt="circular dependencies" className={imgStyle} />
          <p className={paragraphStyle}>
          ...
          <img src={dependencyStages} alt="Dependency stages" className={`${imgStyle} -my-10`} />
          <p className={paragraphStyle}>
          ...
          <img src={zookeeperDependencies} alt="zookeeper depenencies" className={`${imgStyle} -my-10`} />
          <p className={paragraphStyle}>
